...
- id: 4825
  ...
  content: "[...] also took a page from the Oâ€™Reilly book "Designing Web Interfacesâ€\x9D
    plus Axismaps (who helped design GeoCommons Maker!) plus MapTube – and created
    a "dynamic transparencyâ€\x9D [...]"
---
...
<p><span>The fundamental problem with transparency is that<em> the color of semi-transparent map units can shift due to the visibility of color that lies beneath them</em>. This is not at all surprising, but can make the basic legend matching task difficult, obscure the pattern of color on the map, or just as bad, make patterns appear out of nowhere. Hereâ€™s a look at what happens to colors using the same semi-transparent choropleth map units on different backgrounds. These are screen captures from early design mock-ups for Maker!.</span></p>
